...
class RemoteRunner:
    ...
    def build_pipeline(self) -> List[Dict[str, str]]:
        ...
        pip_install_helper = (
            ...
            "_cu_ok=0; "
            "for _cu in https://download.pytorch.org/whl/cu121 https://download.pytorch.org/whl/cu128; do "
            "echo \"--- 正在从 $_cu 安装 torch/torchvision（限时 700 秒，超时自动切换）...\"; "
            "if timeout 600 \"$PYTHON_BIN\" -m pip install --disable-pip-version-check --prefer-binary --index-url \"$_cu\" torch torchvision >/tmp/torch_install.log 2>&1 && \"$PYTHON_BIN\" -c 'import torch; assert torch.cuda.is_available()' >/dev/null 2>&1; then "
        ...
        install_step = (
            ...
            "echo \"GPU 实例缺少可用 CUDA torch（缺失或为 CPU 版），卸载旧版并从官方 CUDA 源安装（每源限时 700s）...\"; "
            "\"$PYTHON_BIN\" -m pip uninstall -y torch torchvision >/dev/null 2>&1 || true; "
            "_cu_ok=0; "
            "for _cu in https://download.pytorch.org/whl/cu121 https://download.pytorch.org/whl/cu128; do "
            "if timeout 700 \"$PYTHON_BIN\" -m pip install --disable-pip-version-check --prefer-binary --index-url \"$_cu\" torch torchvision >/tmp/torch_inst.log 2>&1 && \"$PYTHON_BIN\" -c 'import torch; raise SystemExit(0 if torch.cuda.is_available() else 1)' >/dev/null 2>&1; then "
        ...
        dependency_step = (
            ...
            "echo '检测到仓库需要 CUDA torch 但当前环境缺失或为 CPU 版，先卸载旧版并安装 CUDA 版（官方 cu121 优先，限时 700s/源）...'; "
            "\"$PYTHON_BIN\" -m pip uninstall -y torch torchvision >/dev/null 2>&1 || true; "
            "_cu_ok=0; "
            "for _cu in https://download.pytorch.org/whl/cu121 https://download.pytorch.org/whl/cu128; do "
            "if timeout 600 \"$PYTHON_BIN\" -m pip install --disable-pip-version-check --prefer-binary --index-url \"$_cu\" torch torchvision >/tmp/torch_dep.log 2>&1 && \"$PYTHON_BIN\" -c 'import torch; assert torch.cuda.is_available()' >/dev/null 2>&1; then "
