...
class RemoteRunner:
    ...
    def build_pipeline(self) -> List[Dict[str, str]]:
        ...
        dependency_step = (
            ...
            "echo '检测到仓库需要 torch 但当前环境不可用，自动安装 CUDA 版（cu128→cpu 双源，限时 420s/源）...'; "
            "for _cu in https://download.pytorch.org/whl/cu128 https://download.pytorch.org/whl/cpu; do "
            "if timeout 420 \"$PYTHON_BIN\" -m pip install --disable-pip-version-check --prefer-binary --index-url \"$_cu\" torch torchvision >/tmp/torch_dep.log 2>&1; then "
            "echo \"torch 安装完成（源：$_cu）\"; tail -3 /tmp/torch_dep.log; break; fi; "
            "echo \"源 $_cu 失败，切换下一个...\"; done; "
            "\"$PYTHON_BIN\" -c 'import torch; print(\"torch 可用\", torch.__version__)' >/dev/null 2>&1 && echo 'torch 自动补装成功' || echo 'torch 自动补装失败：请检查网络或在「云服务器」表单确认环境，运行阶段可能报 ImportError'; "
