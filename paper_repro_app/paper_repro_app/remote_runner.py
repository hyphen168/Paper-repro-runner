...
class RemoteRunner:
    ...
    def build_pipeline(self) -> List[Dict[str, str]]:
        ...
        pip_install_helper = (
            ...
            "_cu_ok=0; "
            "for _cu in https://mirrors.aliyun.com/pytorch-wheels/cu121/ https://download.pytorch.org/whl/cu128; do "
            "echo \"--- 正在从 $_cu 安装 torch/torchvision（限时 600 秒，超时自动切换）...\"; "
        ...
        dependency_step = (
            ...
            "if [ \"$_need_torch\" = 1 ] && ! \"$PYTHON_BIN\" -c 'import torch' >/dev/null 2>&1; then "
            "echo '检测到仓库需要 torch 但当前环境不可用，自动安装 CUDA 版（国内源优先，限时 600s/源）...'; "
            "_cu_ok=0; "
            "for _cu in https://mirrors.aliyun.com/pytorch-wheels/cu121/ https://download.pytorch.org/whl/cu128; do "
            ...
            "else echo \"检测到重型框架依赖（$heavy_hit）：环境已可用或无需 torch，跳过自动补装。\"; fi; "
            "fi; "
