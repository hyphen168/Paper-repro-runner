...
- **结果对比图**：成功任务自动渲染「复现 vs 论文宣称」分组柱状图（altair，主题配色），批量任务页提供跨仓库结果总览图；对比行结构化落库，历史数据无缝兼容
- **批量复现**：一次粘贴多篇论文/仓库排队逐篇执行（新增 `batch_id` 与任务队列：同一时刻只跑一个云任务，避免抢占与重复计费；批次可整体取消，失败不阻塞后续任务）
- **SSH 输入健壮性**：服务器地址框任意形态均可即时解析——完整 `ssh -p 端口 user@host` 命令 / `user@host:port` / `host` / ssh 别名 / 多行候选；测试连接、注入公钥、生成配置按钮统一走同一套解析，粘贴登录命令不再报错
- **当地时间**：天气与昼夜按「天气位置当地墙上时钟」计算与展示（Open-Meteo utc_offset_seconds，经度兜底），顶部胶囊显示当地时刻；不再混用机器时区
