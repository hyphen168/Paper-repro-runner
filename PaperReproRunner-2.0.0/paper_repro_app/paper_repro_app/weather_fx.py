"""本地天气粒子特效（物理化渲染）：IP 定位 → Open-Meteo → Canvas 逼真动效。

天气 → 动效（遵循物理直觉）：
- 晴（昼）：真实太阳（核心+径向光晕+缓慢旋转的光芒射线）+ 少量浮尘
- 晴（夜）：弯月 + 闪烁星群 + 偶发流星
- 多云/阴：多层椭圆云朵缓移（夜色下略暗）
- 雨：带风斜角的雨丝，远/中/近三层视差，落地水花溅起
- 雷暴：大雨 + 树枝状折线闪电 + 全屏闪白
- 雪：大小分层、随风摆动的雪花
- 雾：多层横向流动雾带
- 缓存 30 分钟；离线自动降级为安静光点。粒子层在内容层之下（z-index:0），不干扰操作。
"""
...
def describe(weather: Optional[dict]) -> dict:
    """把天气数据翻译成展示配置（label/emoji 保留字段但不用于 UI 图标）。"""
    if not weather:
        return {
            "kind": "calm",
            "label": "天气未知",
    ...
    kind, label, _d, _n = WMO_MAP.get(code, ("clear", "晴", "", ""))
    return {
        "kind": kind,
        "label": label,
...
def _canvas_js(kind: str, is_day: bool, wind: float) -> str:
    """生成物理化天气动画（父文档 body 绘制，pointer-events:none）。"""
    cfg = {"kind": kind, "day": is_day, "wind": float(wind or 0)}
    return _WEATHER_JS.replace("__CFG__", json.dumps(cfg))


def build_particles_html(weather: Optional[dict], override=None) -> str:
    """override=(kind, is_day) 用于界面预览切换；None 表示跟随实时天气。"""
    info = describe(weather)
    kind = override[0] if override else info["kind"]
    is_day = bool(override[1] if override else info["is_day"])
    return '<div style="display:none;">weather-fx</div>' + _canvas_js(kind, is_day, float(info.get("wind") or 0))
...
# 预览切换选项：value -> (kind, is_day, 中文)
WEATHER_PREVIEWS: dict = {
    "auto": ("", True, "自动（跟随当地天气）"),
    "clear:1": ("clear", True, "晴 · 白天（太阳）"),
    "clear:0": ("clear", False, "晴 · 夜晚（星月）"),
    "cloudy:1": ("cloudy", True, "多云"),
    "rain": ("rain", True, "雨"),
    "heavy_rain": ("heavy_rain", True, "大雨"),
    "storm": ("storm", True, "雷暴"),
    "snow": ("snow", True, "雪"),
    "fog": ("fog", True, "雾"),
}
